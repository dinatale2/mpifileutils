...
static void free_file_sections(copy_elem* head) {
    copy_elem* tmp;
    copy_elem* current = head;

    /* free the linked list of structs (copy_elem) */
    while (current != NULL) {
        tmp = current;
        bayer_free(&current);
        current = tmp->next;
...
static copy_elem* identify_file_sections(bayer_flist list, uint64_t chunk_size) {
